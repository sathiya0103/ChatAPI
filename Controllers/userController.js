...
const registerUser = async (req, res) => {
    try {
        ...
        if (user) return res.status(400).json("this email is already exist")
...
const loginUser = async (req, res) => {
    ...
    try {
        let user = await userModel.findOne({ email })
        if (!user) return res.status(400).json("Email must be a valid")

        const isValidPassword = await bcrypt.compare(password, user.password)
        if (!isValidPassword) return res.status(400).json("Invalid emil or password")
